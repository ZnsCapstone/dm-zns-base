#!/usr/bin/env bash
# Validates recovery after one-shot write/checkpoint failpoints.
...
write_failure_case() {
	...
	if dd if="$TMP_DIR/marker" of="$DM_DEV" bs=4K count=1 \
...
wal_crc_case() {
	...
	dd if="$TMP_DIR/marker" of="$DM_DEV" bs=4K count=1 \
...
checkpoint_failure_case() {
	...
	cmp "$TMP_DIR/expected-prefix" "$TMP_DIR/recovered-prefix" ||
		fail "$name lost durable WAL prefix after recovery"
	echo "[OK] checkpoint interruption recovered durable WAL prefix"
}
...
write_failure_case "[1/5] data write succeeds, WAL is never staged" \
	"$FAIL_AFTER_DATA_WRITE"
write_failure_case "[2/5] data write succeeds, WAL page write fails" \
	"$FAIL_BEFORE_WAL_WRITE"
wal_crc_case
checkpoint_failure_case "[4/5] checkpoint fails after SSTable write" \
	"$FAIL_AFTER_SSTABLE_WRITE"
checkpoint_failure_case "[5/5] checkpoint fails after Manifest write" \
